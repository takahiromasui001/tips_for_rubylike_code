...
本章で提案するのは、繰り返し処理におけるEnumerableのメソッドの利用です。
...
上のコードであればブロック引数は「|item|」が該当し、ブロック内ではitemに10を加えたものをpメソッドで出力しています。

Rubyにおける反復処理はこのように、「オブジェクトの各要素に適用したい処理」を用意しておいて、
各要素に対して1つずつ適用する形が好まれます。
(この考え方自体はRuby固有の話ではなくオブジェクト指向ベースの考え方をベースにしたものです。)
eachはその考えを実現できる最も基本的なメソッドです。

==== 戻り値
eachの戻り値はレシーバになります。
ブロック内でどんな処理をしても戻り値は影響を受けません。
上のコードであればeachの戻り値は「list」になります。「p item + 10」の結果は戻り値には何も影響を与えません。

言い換えれば、eachのブロック内の処理結果は、そのままだとeachの外で用いることは出来ません。
...
=== each利用時の課題
eachはRubyの反復処理には欠かせないとても重要なメソッドですが、利用シーンによっては課題もあります。
その課題とは
eachメソッドでは「オブジェクトの各要素に適用したい処理」と同時に、
「処理の結果をどう扱うか？」もブロック内に一緒に記載する必要があることです。

例として @<list>{each2} のコードを用いて説明すると、
...
自分は「オブジェクト内の各要素に適用したい処理」だけをブロックとしてメソッドに渡すイメージです。
...
Enumerableのメソッドは全てeachを用いて定義されていますので、eachを使って作成できます。
(実際のmapと同じものではありませんが、最低限同じ役割を果たすメソッドとして作成します)
...
@<list>{each2}の構造とほぼ一緒です。異なるのはyield(n)を使っていることですが、この点が重要になります。
...
ifに対するunlessと同じような位置付けです。否定条件で表したほうが意図が伝わりやすい場合等に利用します。
